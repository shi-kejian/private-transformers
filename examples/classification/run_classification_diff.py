...
from .src.trainer_diff_bert import Trainer
...
def main():
    ...
    # assert PER_PARAMS_ALPHA == 0 or args.per_layer_alpha == 0, "Only support per params alpha OR per layer alpha"
    optimizer_grouped_parameters = [
        ...
        {'params': alpha_params, 'lr': 0.1, 'eps': training_args.adam_epsilon},
    ]
    ...
    # now check if trainer has successfully added the parameters
    print(trainer.bert_params is not None)
    ...
    # `trainer.optimizer` is not None here, so no optimizer is created -- !! OKAY !!
    ...
    # private case: just ignore for now 
    else:
