...
ALPHA_INIT=5
FIX_LAYER=-1
USE_PER_PARAMS_ALPHA=0
USE_PER_LAYERS_ALPHA=1
...
class Trainer(transformers.Trainer):
    ...
    def train(self, model_path=None, dev_objective=None, dev_objective_key=None):       ##############################################################################################################################################################
        ...
        # total_layers = 14 if "base" in self.args.model_name_or_path else 24
        total_layers = 14

        sparsity_pen = [SPARSITY_PEN] * total_layers  # NB(anon)
        ...
        def get_layer_ind(n):
            if "%s.embeddings"%modelname in n:
                ind = 0
                # print('entered if in get_layer_ind')
            elif "%s.encoder.layer"%modelname in n:
                ind = int(n.replace("%s.encoder.layer."%modelname, "").split(".")[0]) + 1
                # print('entered elif in get_layer_ind')
            else:
                ind = total_layers - 1
                # print('entered else in get_layer_ind')
            return ind
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    for n, p in model.named_parameters():
                        ...
                        if "classifier" in n:
                            ...
                            # print the device of everything of the line above to make sure they are on the same device
                            # print('self.bert_params[n][1].grad.device', self.bert_params[n][1].grad.device)
                            # print('p.grad.data.device', p.grad.data.device)
                            # print('self.bert_params[n][1].device', self.bert_params[n][1].device)
                            # print('p.device', p.device)
                            # print('self.bert_params[n][1].grad', self.bert_params[n][1].grad)
                            # print('p.grad.data', p.grad.data)
                            # print('self.bert_params[n][1]', self.bert_params[n][1])
                            # print('p', p)
                        else:
                            try:
                                # breakpoint()
                                ...
                                embed()
                                self.bert_params[n][2].grad.copy_(p.grad.data * grad_params[n][0].data * grad_params[n][2].data)
                    ...
                    if self.privacy_args.non_private:
                        ...
                        '''
                        11/27 TODO: Need to use diff_prune to clip the gradient of the alpha parameters, not the private learning one 
                        '''
                        ...
                        #################
                        vector_loss_combined  = vector_loss + sum_l0_loss
                        
                        # write vector_loss_combined to a file every 50 steps
                        if step % 50 == 0:
                            with open('vector_loss_combined.txt', 'a') as f:
                                f.write('step: ' + str(step) + '\n')
                                f.write(str(vector_loss_combined) + '\n==========\n')
                            
                        #################

                        # self.optimizer.step(loss=vector_loss, callback=callback)
                        self.optimizer.step(loss=vector_loss_combined, callback=callback)
                    ...
                    if USE_PER_PARAMS_ALPHA == 1:
                        for n,p in self.per_params_alpha.items():
                            p.grad.zero_()
